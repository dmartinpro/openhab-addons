...
- **A timed-out arm/disarm/group command can still leave the UI stale for a poll cycle**: the Diagral cloud API is prone to slow or dropped responses (a `Request timeout` after 10s is common, especially under load). If `mode-control` or a zone's `active` command hits this, the real command may still have been applied server-side even though openHAB reports an error - confirmed live by observing a "failed" disarm settle to the correct state on the very next poll. The binding always triggers an immediate re-poll after any such command regardless of outcome, so the UI corrects itself within a few seconds rather than waiting for the next scheduled interval - but a `WARN`/`ERROR` log line for a command that actually worked is expected under these conditions, not a sign anything is broken.
